...
namespace Nita.ToolKit.Emoji
{
    ...
    public static class FlowDocumentExtensions
    {
        ...
        /// 通用方法，用于在 TextRange 内替换 emoji 图标为 emoji 内联元素。
        /// </summary>
                                                     ...
                                                     SubstituteOptions options)
        {
            ...
            while (cur.CompareTo(range.End) < 0)
            {
                ...
                if (replace_range.Text.Length > 0 && EmojiData.MatchStart.Contains(replace_range.Text[0]))
                {
                    ...
                    // GetNextInsertionPosition()在查找emoji时不可靠，
                    // 因为它有时会在有效序列的中间停止（对于不使用ZWJ的旗帜或某些最近的肤色变化）。
                    // 为了解决这个问题，我们向前查找50个字符。
